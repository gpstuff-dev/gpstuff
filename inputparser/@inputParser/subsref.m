...
function inPar = parse_args (inPar, idx)
  ...
  for i = 1 : numel (fieldnames (inPar.Optional))
    ...
    if ( !feval (inPar.Optional.(name).validator, value) )
      if (ischar (value) ) || (isstruct(value) && ~isempty(value))
  ...
  for i = 1 : (numel (fieldnames (inPar.ParamValue)) + numel (fieldnames (inPar.Switch)))
    if numel(args)==1 && isstruct(args{1}) %&& isequal(fieldnames(args{1})(:), inPar.copy(:))
      ## input parser given structure containing ParamValues, modify args
      tmpargs=args{1};
      args={};
      names=fieldnames(tmpargs);
      for j = 1 : numel(names)
        args{1,end+1} = names{j};
        args{1,end+1} = getfield(tmpargs, names{j});
      endfor
    endif
    if ( numel (args) < 2)
  ...
  elseif ( numel (args) ) && ~isempty(all(struct2cell(args{:})))
...
function inPar = validate_args (method, inPar, name, val, def = false)
  ...
  ## allow empty default as in Matlab
  if ( isa (val, 'function_handle') && !strcmpi (method, 'Required') && !isempty(def) && !feval (val, def) )
