...
\title{Bayesian Modeling with Gaussian Processes using the
  \proglang{MATLAB} Toolbox \pkg{GPstuff} (v3.4)}
...
\author{\name Jarno Vanhatalo\thanks{Work done mainly while at the
  ...
  \name Jaakko Riihim\"aki \email jaakko.riihimaki@aalto.fi \\
  \name Jouni Hartikainen \email jouni.hartikainen@aalto.fi \\
  \name Pasi Jyl\"anki \email pasi.jylanki@aalto.fi \\
  \name Ville Tolvanen \email ville.tolvanen@aalto.fi \\
  \name Aki Vehtari \email aki.vehtari@aalto.fi \\
...
Gaussian processes (GP) are powerful tools for probabilistic
  ...
  versatile collection of computational tools for GP models. It has
  been implemented for Linux and Windows \proglang{MATLAB} and
  includes, among others, various inference methods, sparse
  approximations and tools for model assessment. In this work, we
  review these tools and demonstrate the use of \pkg{GPstuff} in
  several models.
\end{abstract}
...
3.3) which implements a collection of inference methods and tools for
inference for various Gaussian process (GP) models. The toolbox is
written for \proglang{MATLAB} \citep{MATLAB:2010} and it is available
from \url{http://becs.aalto.fi/en/research/bayes/gpstuff/}.
...
spatial statistics
\citep{Best+Richardson+Thomson:2005,Kaufman+Schervish+Nychka:2008,Banerjee+Gelfand+Finley+Sang:2008},
...
\citep{Stegle+Fallert+MacKay+Brage:2008,Vanhatalo+Pietilainen+Vehtari:2010},
...
\citep{Tokdar+Ghosh:2007,Tokdar+Zhu+Ghosh:2010} to name a few.
...
\citep{Vanhatalo+Jylanki+Vehtari:2009,Vanhatalo+Pietilainen+Vehtari:2010,
  Vanhatalo+Vehtari:2010,Jylanki+Vanhatalo+Vehtari:2011}.
...
\pkg{FBM} is provided in the Appendix \ref{appendix:compasison}.
...
This work concentrates on discussing the essential theory behind the
implementation of \pkg{GPstuff}. We explain important parts of the
code, but the full code demonstrating the important features of the
package (including also data creation and such), are included in the
demonstration files \code{demo\_*} to which we refer in the text.
...
at the mode \citep{Gelman+Carlin+Stern+Rubin:2004,Rasmussen+Williams:2006}: 
%
\begin{equation}
\mb{\Sigma}^{-1} = -\nabla\nabla \log
...
for example,
\citep{Gelman+Carlin+Stern+Rubin:2004,Robert+Casella:2004} for
...
predictive performance. \pkg{GPstuff} provides four basic approaches:
marginal likelihood, cross-validation, deviance information
criterion and widely applicable information criterion.
...
approximation using linear response approach (to be submitted),
...
sampling \citep{Gelfand+Dey+Chang:1992,Vehtari+Lampinen:2002},
which is also implemented in \code{gp\_loopred}.

To reduce computation time, in $k$-fold-CV, $1/k$ part of the training
data is left out, and then the predictive distribution is computed
with a model that is fitted to all of the observations except that
part.  Since the $k$-fold-CV predictive densities are based on smaller
training data sets than the full data set, the estimate is slightly
biased. In \pkg{GPstuff} first order bias correction proposed by
\citet{Burman:1989} is used.  \pkg{GPstuff} provides \code{gp\_kfcv},
which computes $k$-fold-CV and bias-corrected $k$-fold-CV with
log-score and root mean squared error (RMSE). The function
\code{gp\_kfcv} provides also basic variance estimates for the
predictive performance estimates.  Variance is computed from the
estimates for each fold ~\citep[see, e.g.,][]{Dietterich:1998}.  See
\citep{Vehtari+Lampinen:2002} for more details on estimating the
...
In this section, we discuss three more example applications where
\pkg{GPstuff} has been used. The purpose of this section is to
introduce few other likelihood functions implemented in \pkg{GPstuff}
...
\begin{figure}
  ...
  \end{center}\caption{An example of regression with outliers from
    ...
    Student-$t$ likelihood.  The real function is plotted with
    black line.}
...
described by][]{Neal:1998}. The Student-$t$ likelihood is
...
\subsection{Disease mapping}\label{sec_spatial_demo1}

Spatial epidemiology concerns both describing and understanding the
spatial variation in the disease risk in geographically referenced
health data. One of the most common tasks in spatial epidemiology is
disease mapping, where the aim is to describe the overall disease
distribution on a map and, for example, highlight areas of elevated or
lowered mortality or morbidity risk
...
The Poisson likelihood is implemented in \code{likelih\_poisson} and
it is
%
\begin{equation}
...
\subsubsection{Negative binomial likelihood}\label{sec_spatial_demo2}
...
\citep{Gelman+Carlin+Stern+Rubin:2004}. In \pkg{GPstuff} it is
parametrized as
%
\begin{equation}
...
\subsection{Survival analysis with Weibull model}
...
used in \citet{Martino:2011}. The observation model is defined as
\begin{equation}
  \y |\f,\mathbf{z}, r  \sim \prod_{i=1}^n r^{1-z_i} \exp \left( (1-z_i)f(\x_i)+(1-z_i)(r-1)\log(y_i)-\exp(f(\x_i))y_i^r \right),
\end{equation}
where $\mathbf{z}$ is a vector of censoring indicators with $z_i = 0$ for
uncensored event and $z_i = 1$ for right censored event for
observation $i$. The use of Weibull model with survival data is
demonstrated in \code{demo\_survival\_weibull}.
...
Quantile regression is used for estimating quantiles of the response variable as a function of input variables \citep{Boukouvalas:2012}. 

The likelihood is defines as
\begin{align}
p(y | f, \sigma, \tau) = \frac{\tau(1-\tau)}{\sigma}\exp\left[-\frac{y-f}{\sigma}(\tau - I(y \le f))\right],
\end{align}
where $\tau$ is the quantile of interest and $\sigma$ is the standard deviation. $I(y \le f)$ is 1 if the condition inside brackets is true and 0 otherwise. Because the logarithm of the likelihood is not twice differentiable at the mode, Laplace approximation cannot be used for inference with Quantile Regression. Quantile Regression with EP and MCMC is demonsrated in \code{demo\_qgp} with toy data.
...
and density regression. GPstuff includes implementation based on
...
Input-dependent noise model \citep{Goldberg:1997} can be used to model regression data. The model assumes Gaussian distributed data with latent variables defining both mean and variance of the Gaussian distribution instead of just the mean like in the standard Gaussian likelihood. By setting independent GP priors for the two latent variables, it is possible to infer non-constant noise, e.g. input-dependent, in the data. 

The likelihood is defined as
\begin{align}
  p(\y|\f^{(1)}, \f^{(2)}, \sigma^2)=\prod_{i=1}^n N(y_i | f_i^{(1)}, \sigma^2 e^{f_i^{(2)}}),
\end{align}
with latent function $f^{(1)}$ defining the mean and $f^{(2)}$ defining the variance. Input-dependent noise is demonstrated in \code{demo\_inputdependentnoise} with both heteroscedastic and constant noise.
% missing description
...
or likelihood, is written in a \proglang{MATLAB} m-file which contains
...
results are shown for Mátern class of covariance functions but
...
The research leading to \pkg{GPstuff} was funded by the Academy of
Finland, Finnish Funding Agency for Technology and Innovation
(TEKES), and the Graduate School in Electronics and
Telecommunications and Automation (GETA). The first author also
thanks the Finnish Foundation for Economic and Technology Sciences
KAUTE, Finnish Cultural Foundation, and Emil Aaltonen Foundation
for supporting his post graduate studies during which the code
package was prepared. 

The package contains pieces of code written by other people than
the authors. In the Department of Biomedical Engineering and
Computational Science at Aalto University these persons are (in
alphabetical order): Toni Auranen, Roberto Calandra, Pasi Jylänki,
Tuomas Nikoskinen, Eero Pennala, Heikki Peura, Ville Pietiläinen,
Markus Siivola, Simo Särkkä and Ville Tolvanen. People outside
Aalto University are (in alphabetical order): Christopher M.
Bishop, Timothy A. Davis, Matthew D. Hoffman, Kurt Hornik, Dirk-Jan
Kroon, Iain Murray, Ian T. Nabney, Radford M. Neal and Carl E.
Rasmussen. We want to thank them all for sharing their code under a
...
GPML and FBM}\label{appendix:compasison}
...
%      that the method is available for all model blocks.\label{table:comparison}}\\
   ...
   \hline
   number of elementary functions & 13 & 6 & 13 \\
   ...
   Log-Gaussian  & MCMC/EP & & \\
   Log-logistic  & MCMC/EP & & \\
   ...
   density regression  & x &  & \\
   input dependent noise   & x &  & \\
   input dependent overdispersion in Weibull   & x &  & \\
   ...
   FIC & x &  & \\
   FIC+CS & x &  & \\
   FITC & x & MCMC/EP/Laplace & \\
   PIC & x &  & \\
   VAR & Gaussian/EP only &  & \\
   DTC & Gaussian/EP only &  & \\
   SOR & Gaussian/EP only &  & \\
    ...
    Scaled HMC, Elliptical slice sampling  & x & x & \\
    \multicolumn{4}{l}{\textbf{Hyperparameter inference}} \\
    \hline
    Type II ML  & x & x & x \\
    Type II MAP  & x & & x \\
    LOO-CV, Laplace/EP LOO-CV  & x & & \\
    Metropolis, HMC  & x & x & \\
 ...
 %   LBFGS & x & x & \\
 %   any Matlab optimization function & x &  & \\
%   \textbf{Other} &  &  & \\
%   \hline 
   \caption[The comparison of features in GPstuff, GPML and FBM
   toolboxes.]{The comparison of features in GPstuff, GPML and FBM
...
\citep{Neal:1996a,Williams:1996,Rasmussen+Williams:2006}. A
...
\citep{Neal:1997,Gelman+Carlin+Stern+Rubin:2004}. 
...
for example, in \citet{Gelman+Carlin+Stern+Rubin:2004}[p. 24].
...
in Matlab were too inefficient).
