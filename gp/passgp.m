...
% License.txt, included with the software, for details.
  ...
  ip=iparser(ip,'addParamValue','npass', 3, @(x) isscalar(x) && x > 0);
  ip=iparser(ip,'addParamValue','ninit', 100, @(x) isscalar(x) && x > 0);
  ip=iparser(ip,'addParamValue','nsub', 10, @(x) isscalar(x) && x > 0);
  ip=iparser(ip,'addParamValue','pinc', 0.5, @(x) isscalar(x) && x > 0);
  ip=iparser(ip,'addParamValue','pdel', 0.99, @(x) isscalar(x) && x > 0);
  ip=iparser(ip,'addParamValue','pexc', 0.1, @(x) isscalar(x) && x > 0);
  ip=iparser(ip,'addParamValue','opt', [], @(x) isstruct(x));
  ip=iparser(ip,'addParamValue','inf_method', 'MAP', @(x) ismember(x, {'MAP', 'MCMC', 'IA'}));
  ip=iparser(ip,'addParamValue','fixed', 'off', @(x) ismember(x,{'on','off'}));
