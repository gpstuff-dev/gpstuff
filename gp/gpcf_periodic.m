...
% License.txt, included with the software, for details.
  ...
  ip=iparser(ip,'addParamValue','N',6, @(x) isscalar(x) && mod(x,1)==0);
  ip=iparser(ip,'addParamValue','N_sexp',6, @(x) isscalar(x) && mod(x,1)==0);
  ip=iparser(ip,'addParamValue','valid',0, @(x) isscalar(x) && (x==0||x==1));
