...
%      K_1,...,K_C whose elements are given by K_ij = k(x_i, x_j | th).
%      The function k(x_i, x_j | th) is covariance 
...
L = strrep(S,'demo_multiclass.m','demos/cdata');
x=load(L);
...
likelih = likelih_softmax('init');

% Create the GP data structure
gp = gp_init('init', 'FULL', likelih, {gpcf1}, {},'jitterSigma2', 1e-6);
...
% gradcheck(w0, @gpla_softmax_e, @gpla_softmax_g, gp, x, y);

w_opt=scg2(@gpla_softmax_e, w0, scg_opt, @gpla_softmax_g, gp, x, y);
gp=gp_unpak(gp,w_opt);


% make the prediction for test points
[Ef, Varf, pt] = la_softmax_pred(gp, x, y, xt);

% calculate the percentage of misclassified points
tt = pt==repmat(max(pt,[],2),1,size(pt,2));
...
[Ef, Covf, pg] = la_softmax_pred(gp, x, y, xtg);
