function [record, gp, opt] = gp2_mc(gp, x, y, varargin)
%GP_MC  Markov chain Monte Carlo sampling for Gaussian process models
%
%  Description
%    [RECORD, GP, OPT] = GP2_MC(GP, X, Y, OPTIONS) Takes the Gaussian 
...
%          record fields of:
  ...
  record.etr(ri,:)= gpmc_e(gp_pak(gp), gp, x, y, gp.latentValues, z);
  % Set rejects
