function [Eft, Varft, lpyt, Eyt, Varyt] = gp2_pred(gp, x, y, varargin)
%GP2_PRED  Make predictions with Gaussian process 
%
%  Description
%    [EFT, VARFT] = GP2_PRED(GP, X, Y, XT, OPTIONS)
...
%
%    [EFT, VARFT, LPYT] = GP2_PRED(GP, X, Y, XT, 'yt', YT, ...)
...
% 
%    [EFT, VARFT, LPYT, EYT, VARYT] = GP2_PRED(GP, X, Y, XT, OPTIONS)
...
switch gp.type
  case {'LGP', 'LGPC', 'Coxph'}
    error('gp2_pred not implemented for this type of likelihood');
  case {'Softmax', 'Multinom'}
    [tn,nout] = size(y);
  otherwise
    tn = size(y,1);
    nout=size(gp.latentValues,2)/size(y,1);
end

if nargout > 2 && isempty(yt)
  error('GP2_PRED -> To compute LPYT, the YT has to be provided.')
end
