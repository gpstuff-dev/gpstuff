...
[mlpd_cv(1), Var_lpd_cv(1), mrmse_cv(1), Var_rmse_cv(1)] = gp_kfcv(gp, x, y, 'MAP_scg2', opt);
...
opt.nsamples= 50; 
[mlpd_cv(2), Var_lpd_cv(2), mrmse_cv(2), Var_rmse_cv(2)] = gp_kfcv(gp, x, y, 'MCMC', opt, 'covariance', 0);
...
[mlpd_cv(3), Var_lpd_cv(3), mrmse_cv(3), Var_rmse_cv(3)] = gp_kfcv(gp, x, y, 'IA', opt, 'covariance', 0);
...
% Evaluate the 10-fold cross validation results. 
[mlpd_cv(4), Var_lpd_cv(4), mrmse_cv(4), Var_rmse_cv(4)] = gp_kfcv(gp_fic, x, y, 'MAP_scg2', opt, 'covariance', 0);
...
% should be run with larger sample size. We also set the save option to 0.
opt.nsamples= 50; 
[mlpd_cv(5), Var_lpd_cv(5), mrmse_cv(5), Var_rmse_cv(5)] = gp_kfcv(gp_fic, x, y, 'MCMC', opt, 'covariance', 0);
...
% Then the 10 fold cross-validation.
[mlpd_cv(6), Var_lpd_cv(6), mrmse_cv(6), Var_rmse_cv(6)] = gp_kfcv(gp_fic, x, y, 'IA', opt, 'covariance', 0);
...
w=scg2(fe, w, opt, fg, gp_pic, x, y, param);       % do the optimization
...
% Evaluate the 10-fold cross validation results. 
[mlpd_cv(7), Var_lpd_cv(7), mrmse_cv(7), Var_rmse_cv(7)] = gp_kfcv(gp_pic, x, y, 'MAP_scg2', opt, 'covariance', 0);
...
% should be run with larger sample size. We also set the save option to 0.
opt.nsamples= 50; 
[mlpd_cv(8), Var_lpd_cv(8), mrmse_cv(8), Var_rmse_cv(8)] = gp_kfcv(gp_pic, x, y, 'MCMC', opt, [], 0);
...
% Then the 10 fold cross-validation.
[mlpd_cv(9), Var_lpd_cv(9), mrmse_cv(9), Var_rmse_cv(9)] = gp_kfcv(gp_pic, x, y, 'IA', opt, [], 0);
...
w=scg2(fe, w, opt, fg, gp_csfic, x, y, param);       % do the optimization
gp_csfic = gp_unpak(gp_csfic,w, param);     % Set the optimized hyperparameter values back to the gp structure
...
% Evaluate the 10-fold cross validation results. 
[mlpd_cv(10), Var_lpd_cv(10), mrmse_cv(10), Var_rmse_cv(10)] = gp_kfcv(gp_csfic, x, y, 'MAP_scg2', opt, 'covariance', 0);
...
% Then the 10 fold cross-validation.
[mlpd_cv(12), Var_lpd_cv(12), mrmse_cv(12), Var_rmse_cv(12)] = gp_kfcv(gp_csfic, x, y, 'IA', opt, 'covariance', 0);
