        ...
        if nargout < 3
            ...
            if isfield(gp, 'latentValues')
                ...
                if any(diag(Varf(:,:,i1))<=0)
                    ...
                    if ~isempty(nzero)
                        warning('gp_mc: Some of the Varf diagonal elements are less than or equal to zero. Those are set to 1e-12.') 
                        for j=1:length(nzero)
                            ni = nzero(j);
                            Varf(ni,ni,i1) = 1e-12;    % Ensure positiviness, which may be a problem with FIC
                        end
                    end
                end
