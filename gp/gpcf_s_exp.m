function gpcf = gpcf_s_exp(do, varargin)
%GPCF_S_EXP	Create a squared exponential covariance function for Gaussian Process.
%
%	Description
%
%	GPCF = GPCF_S_EXP('INIT', NIN) Create and initialize squared exponential 
%       covariance function fo Gaussian process 
%
%	The fields and (default values) in GPCF_S_EXP are:
%	  type           = 'gpcf_s_exp'
...
%                          (gpcf_s_exp_recappend)
%
%	GPCF = GPCF_S_EXP('SET', GPCF, 'FIELD1', VALUE1, 'FIELD2', VALUE2, ...)
...
% License.txt, included with the software, for details.
    ...
    if strcmp(do, 'init')
        ...
        gpcf.fh_pak = @gpcf_s_exp_pak;
        gpcf.fh_unpak = @gpcf_s_exp_unpak;
        gpcf.fh_e = @gpcf_s_exp_e;
        gpcf.fh_ghyper = @gpcf_s_exp_ghyper;
        gpcf.fh_gind = @gpcf_s_exp_gind;
        gpcf.fh_cov = @gpcf_s_exp_cov;
        gpcf.fh_covvec = @gpcf_s_exp_covvec;
        gpcf.fh_trcov  = @gpcf_s_exp_trcov;
        gpcf.fh_trvar  = @gpcf_s_exp_trvar;
        gpcf.fh_recappend = @gpcf_s_exp_recappend;
    ...
    function w = gpcf_s_exp_pak(gpcf, w)
    %Gpcf_S_Exp_PAK	 Combine GP covariance function hyper-parameters into one vector.
    %
    %	Description
    ...
    function [gpcf, w] = gpcf_s_exp_unpak(gpcf, w)
    %GPCF_S_EXP_UNPAK  Separate GP covariance function hyper-parameter vector into components. 
    %
    %	Description
    %	GP = GPCF_S_EXP_UNPAK(GP, W) takes an Gaussian Process data structure GP
    ...
    function eprior =gpcf_s_exp_e(gpcf, x, t)
    %GPCF_S_EXP_E	Evaluate prior contribution of error of covariance function SE.
    %
    %	Description
    %	E = GPCF_S_EXP_E(W, GP, X, T) takes a gp data structure GPCF together
    ...
    function [g, gdata, gprior]  = gpcf_s_exp_ghyper(gpcf, x, t, g, gdata, gprior, varargin)
    %GPCF_S_EXP_GHYPER    Evaluate the gradient of error for EXP covariance function.
    %                   with respect to the hyperparameters
    %
    %	Descriptioni
    %	G = GPCF_S_EXP_GHYPER(W, GPCF, X, T, G, GDATA, GPRIOR, VARARGIN) takes a gp 
    ...
    %
    %	[G, GDATA, GPRIOR] = GPCF_S_EXP_G(GP, P, T) also returns separately  the
...
% $$$                 H = H2 + H2' + sparse(1:n,1:n,H_diag,n,n);
    ...
    function [DKuu_u, DKuf_u]  = gpcf_s_exp_gind(gpcf, x, t, varargin)
    ...
    function C = gpcf_s_exp_cov(gpcf, x1, x2)
    ...
    function C = gpcf_s_exp_trcov(gpcf, x)
    ...
    function C = gpcf_s_exp_covvec(gpcf, x1, x2, varargin)
    % GPCF_S_EXP_COVVEC     Evaluate covariance vector between two input vectors. 
    %
    %         Description
    %         C = GPCF_S_EXP_COVVEC(GP, TX, X) takes in Gaussian process GP and two
    ...
    function C = gpcf_s_exp_trvar(gpcf, x)
    ...
    function reccf = gpcf_s_exp_recappend(reccf, ri, gpcf)
    % RECAPPEND - Record append
    %          Description
    %          RECCF = GPCF_S_EXP_RECAPPEND(RECCF, RI, GPCF) takes old covariance 
    ...
    % Initialize record
        if nargin == 2
            ...
            % Set the function handles
            reccf.fh_pak = @gpcf_s_exp_pak;
            reccf.fh_unpak = @gpcf_s_exp_unpak;
            reccf.fh_e = @gpcf_s_exp_e;
            reccf.fh_g = @gpcf_s_exp_g;
            reccf.fh_cov = @gpcf_s_exp_cov;
            reccf.fh_covvec = @gpcf_s_exp_covvec;
            reccf.fh_trcov  = @gpcf_s_exp_trcov;
            reccf.fh_trvar  = @gpcf_s_exp_trvar;
            %  gpcf.fh_sampling = @hmc2;
            %  reccf.sampling_opt = hmc2_opt;
            reccf.fh_recappend = @gpcf_s_exp_recappend;  
            return
        end
