...
switch gp.type
  case 'FULL'    
    ...
    [e,edata,eprior,param]=gpsvi_e(w,gp,x,y);
    u=gp.X_u;
    nu=size(u,1);
    n=size(x,1);
    gp=gp_unpak(gp,w);
...
%     gdata(1:nu)=(1./s2.*(K_uu\(K_fu'*y)) - theta1);
    ...
    elseif isequal(gp.lik.type, 'Probit')
      if isfield(gp, 'lik_mono') && isfield(gp, 'derivobs') && gp.derivobs==1
        ss=[s2*ones(size(x,1),1); beta.*ones(size(K_fu,1)-size(x,1),1)];
        if isfield(gp.lik_mono, 'nu')
...
if ~isempty(strfind(gp.infer_params, 'likelihood')) && ~isempty(gp.lik.p.sigma2) 
  % Prior contribution
...
if ~isempty(strfind(gp.infer_params, 'likelihood')) && isfield(gp, 'lik_mono') ...
