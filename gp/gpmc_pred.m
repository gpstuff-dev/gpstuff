function [Ef, Varf, lpy, Ey, Vary] = gpmc_pred(gp, x, y, xt, varargin)
...
%    returns also the predictive density PY of the observations YT
...
% License.txt, included with the software, for details.
  ...
  switch nargout % ugly...
    ...
    case 3
      [Efs, Varfs, lpys] = gpmc_preds(gp, x, y, xt, varargin{:});
      Ef=mean(Efs,2);
      Varf=mean(Varfs,2) + var(Efs,0,2);
      lpy=mean(lpys,2);
    case 4
      [Efs, Varfs, lpys, Eys] = gpmc_preds(gp, x, y, xt, varargin{:});
      Ef=mean(Efs,2);
      Varf=mean(Varfs,2) + var(Efs,0,2);
      Ey=mean(Eys,2);
      lpy = mean(lpys,2);
    case 5
      [Efs, Varfs, lpys, Eys, Varys] = gpmc_preds(gp, x, y, xt, varargin{:});
      Ef=mean(Efs,2);
      Varf=mean(Varfs,2) + var(Efs,0,2);
      Ey=mean(Eys,2);
