...
% $$$     end
    ...
    function [g, gdata, gprior]  = gpcf_matern52_g(gpcf, x, t, g, gdata, gprior, invC, varargin)
    %GPCF_MATERN52_G Evaluate gradient of error for SE covariance function.
    %
    %	Descriptioni
    %	G = GPCF_MATERN52_G(W, GPCF, X, T, invC, B) takes a gp hyper-parameter  
    ...
    if isfield(gpcf, 'sparse')
        ...
        switch gpcf.sparse
          case 'FIC'
            Kuum = gpcf_matern52_trcov(gpcf, u);
            Kvffm=ones(n,1).*gpcf.magnSigma2;  
            Kfum = gpcf_matern52_cov(gpcf, x, u);
            ma2 = gpcf.magnSigma2;
            ...
            if length(gpcf.lengthScale) == 1
                ...
                Kful = ma2./3.*(5.*dist.*s^2 + 5.*sqrt(5.*dist).*dist.*s.^3).*exp(-sqrt(5.*dist).*s);
                Kuul = ma2./3.*(5.*dist2.*s^2 + 5.*sqrt(5.*dist2).*dist2.*s.^3).*exp(-sqrt(5.*dist2).*s);;
                ...
                for i=1:m  
                    Kful = ma2.*s(i).*((5+5.*sqrt(5).*dist)/3).*(gminus(x(:,i),u(:,i)')).^2.*exp(-sqrt(5).*dist);
                    Kuul = ma2.*s(i).*((5+5.*sqrt(5).*dist2)/3).*(gminus(u(:,i),u(:,i)')).^2.*exp(-sqrt(5).*dist2);
                    % First three rows evalute the y'*inv*(d/dt)*inv*y part to the gradient
                    bdl = (2.*(invC*Kful)-(invC*iKuuKuf')*Kuul)*(iKuuKuf*invC');
                    Kuul = iKuuKuf'*Kuul;  % f x u
                    Kuul = Kuul(:);        
...
% $$$ function [Cdm, Cdl] = gpcf_matern52_gradhelp(gpcf, x, u)
% $$$ % GP_MATERN52_GRADHELP     Evaluate help matrices for gradient evaluation.
% $$$ %
% $$$ %         Description
% $$$ %         C = GP_MATERN52_GRADHELP(GP, TX) takes in covariance function of a Gaussian
% $$$ %         process GP and matrix TX that contains training input vectors to 
% $$$ %         GP. Returns covariance matrix C. Every element ij of C contains  
% $$$ %         covariance between inputs i and j in TX 
% $$$ %
...
% $$$   % Here we take advantage of the 
% $$$   % symmetry of covariance matrix
% $$$   Cdl=zeros(n1,n1,m1);    % help matrix for gpcf_matern52_g
% $$$   Cdm=zeros(n1,n1);    % help matrix for gpcf_matern52_g
% $$$   for i1=2:n1
% $$$     i1n=(i1-1)*n1;
% $$$     for i2=1:i1-1
% $$$       ii=i1+(i2-1)*n1;
...
% $$$ end  
% $$$          % Cdm is used in the partial derivatives of gpcf_matern52
% $$$          % with respect to log(magnSigma2) (needed in HMC)
% $$$ 
% $$$ % evaluate the help matrix Cdl. This is used in the partial 
% $$$ % derivatives of gpcf_matern52 with respect to log(lenghtScale) (needed in HMC)
% $$$ if nargout >1 
% $$$   [n2,m2]=size(u);
% $$$   for i3=1:m1
