...
%         gdata_lik = - DL_sigma - 0.5.*sum(s2t.*DW_sigma) - s2'*s3;
    ...
    case {'DTC', 'SOR'}
      ...
      W = -gp.lik.fh.llg2(gp.lik, y, f, 'latent', z);
      sqrtW = sqrt(W);
%       K=K_fu*inv(K_uu)*K_fu';
%       LL=chol(eye(n)+diag(sqrtW)*K*diag(sqrtW),'lower');
%       R=diag(sqrtW)*(LL'\(LL\diag(sqrtW)));
%       C=LL\(diag(sqrtW)*K);
...
%       B3 = sWKfu;
      ...
      L2 = repmat(sqrtW,1,m).*(sWKfu/chol(A));
%       iLa2W = W;
...
%       s2t = s2t - (La1.*iLa2W.*La1 - sum(C2.^2)' + sum(B'.*((B*(repmat(iLa2W,1,m).*B'))*B)',2)...
%                    - sum(C1.^2)' + 2*La1.*iLa2W.*BB - 2*La1.*sum(L2.*C1',2));
      s2t = BB - (sum(B'.*((B*(repmat(W,1,m).*B'))*B)',2) - sum(C1.^2)');

      s2 = 0.5*s2t.*gp.lik.fh.llg3(gp.lik, y, f, 'latent', z);
      g = gp.lik.fh.llg(gp.lik, y, f, 'latent', z);
...
%             DK=DKuf'*inv(K_uu)*K_fu' - K_fu*inv(K_uu)*DKuu*inv(K_uu)*K_fu' + K_fu*inv(K_uu)*DKuf;
%             gdata(i1) = 0.5*a'*DK*a; 
%             gdata(i1) = gdata(i1) - 0.5*trace(R*DK);
%             b = DK*g;
            KfuiKuuDKuu = iKuuKuf'*DKuu;
            % 0.5* a'*dK*a, where a = K\f
            gdata(i1) = 0.5.*((a'*DKuf'-(a'*KfuiKuuDKuu))*(iKuuKuf*a) + ...
                               a'*iKuuKuf'*(DKuf*a)) ;

            % trace( inv(inv(W) + K) * dQ) )
            gdata(i1) = gdata(i1) + 0.5.*sum(sum(L2'.*(L2'*DKuf'*iKuuKuf - L2'*KfuiKuuDKuu*iKuuKuf + L2'*iKuuKuf'*DKuf)));
            gdata(i1) = gdata(i1) - 0.5.*sum(W.*(2.*sum(DKuf'.*iKuuKuf',2) - sum(KfuiKuuDKuu.*iKuuKuf',2)));
%             gdata(i1) = gdata(i1) + 0.5.*(2.*sum(LL.*sum(DKuf'.*iKuuKuf',2)) - sum(LL.*sum(KfuiKuuDKuu.*iKuuKuf',2)));
%             
            b = (DKuf' - KfuiKuuDKuu)*(iKuuKuf*g) + iKuuKuf'*(DKuf*g);
