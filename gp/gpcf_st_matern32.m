function gpcf = gpcf_st_matern32(do, varargin)
%GPCF_ST_MATERN32	Create a spatio-temporal Matern32 covariance function for Gaussian Process
%
%	Description
%
%	GPCF = GPCF_ST_MATERN32('INIT', NIN) Create and initialize spatio-temporal 
%       Matern32 covariance function. This differs from basic Matern32 so that here the 
%       length-scale is same for both spatial components (the first two inputs) and 
%       different for temporal component(s) (third input and inputs afer that).
%
%	The fields and (default values) in GPCF_ST_MATERN32 are:
%	  type           = 'gpcf_st_matern32'
...
%                          (gpcf_st_matern32_recappend)
%
%	GPCF = GPCF_ST_MATERN32('SET', GPCF, 'FIELD1', VALUE1, 'FIELD2', VALUE2, ...)
...
if strcmp(do, 'init')
    ...
    % function [g, gdata, gprior]  = gpcf_st_matern32_g(gpcf, x, t, g, gdata, gprior, invC, varargin)
    % function reccf = gpcf_st_matern32_recappend(reccf, ri, gpcf)
    ...
    gpcf.fh_pak = @gpcf_st_matern32_pak;
    gpcf.fh_unpak = @gpcf_st_matern32_unpak;
    gpcf.fh_e = @gpcf_st_matern32_e;
    gpcf.fh_g = @gpcf_st_matern32_g;
    gpcf.fh_cov = @gpcf_st_matern32_cov;
    gpcf.fh_trcov  = @gpcf_st_matern32_trcov;
    gpcf.fh_trvar  = @gpcf_st_matern32_trvar;
    %  gpcf.fh_sampling = @hmc2;
    %  gpcf.sampling_opt = hmc2_opt;
    gpcf.fh_recappend = @gpcf_st_matern32_recappend;
...
if strcmp(do, 'set')
    ...
    function w = gpcf_st_matern32_pak(gpcf, w)
    %Gpcf_St_Matern32_PAK	 Combine GP covariance function hyper-parameters into one vector.
    %
    %	Description
    ...
    function [gpcf, w] = gpcf_st_matern32_unpak(gpcf, w)
    %GPCF_ST_MATERN32_UNPAK  Separate GP covariance function hyper-parameter vector into components. 
    %
    %	Description
    %	GP = GPCF_ST_MATERN32_UNPAK(GP, W) takes an Gaussian Process data structure GP
    ...
    function eprior =gpcf_st_matern32_e(gpcf, x, t)
    %GPCF_ST_MATERN32_E	Evaluate prior contribution of error of covariance function SE.
    %
    %	Description
    %	E = GPCF_ST_MATERN32_E(W, GP, X, T) takes a gp data structure GPCF together
...
% $$$     end
    ...
    function [g, gdata, gprior]  = gpcf_st_matern32_g(gpcf, x, t, g, gdata, gprior, invC, varargin)
    %GPCF_ST_MATERN32_G Evaluate gradient of error for SE covariance function.
    %
    %	Descriptioni
    %	G = GPCF_ST_MATERN32_G(W, GPCF, X, T, invC, B) takes a gp hyper-parameter  
    ...
    %
    %	[G, GDATA, GPRIOR] = GPCF_ST_MATERN32_G(GP, P, T) also returns separately  the
    ...
    if isfield(gpcf, 'sparse')
        ...
        switch gpcf.sparse
          case 'FIC'
            Kuum = gpcf_st_matern32_trcov(gpcf, u);
            Kvffm=ones(n,1).*gpcf.magnSigma2;  
            Kfum = gpcf_st_matern32_cov(gpcf, x, u);
            ma2 = gpcf.magnSigma2;
            s = 1./gpcf.lengthScale.^2;
            dist = 0;
            dist2 = 0;
            dist = dist + s(1).*(gminus(x(:,1),u(:,1)')).^2 + s(1).*(gminus(x(:,2),u(:,2)')).^2;
            dist2 = dist2 + s(1).*(gminus(u(:,1),u(:,1)')).^2 + s(1).*(gminus(u(:,2),u(:,2)')).^2;
            for i=3:m
                dist = dist + s(i-1).*(gminus(x(:,i),u(:,i)')).^2;
                dist2 = dist2 + s(i-1).*(gminus(u(:,i),u(:,i)')).^2;
            end
            dist = sqrt(dist); dist2 = sqrt(dist2);
            
            Kful = 3.*ma2.*s(1).*((gminus(x(:,1),u(:,1)')).^2+(gminus(x(:,2),u(:,2)')).^2).*exp(-sqrt(3).*dist);
            Kuul = 3.*ma2.*s(1).*((gminus(u(:,1),u(:,1)')).^2+(gminus(u(:,2),u(:,2)')).^2).*exp(-sqrt(3).*dist2);
            ...
            Bdl(1) = bdl - 2*sum(B.*Kful)+sum(B.*Kuul);
            % evalute the tr(inv*d/dt) part to the gradient
            Cdl(1) = 2.*sum(A.*Kful)-sum(A.*Kuul(:))-2.*sum(W.*Kful)+sum(W.*Kuul);
            
            for i=3:m  
                Kful = 3.*ma2.*s(i-1).*(gminus(x(:,i),u(:,i)')).^2.*exp(-sqrt(3).*dist);
                Kuul = 3.*ma2.*s(i-1).*(gminus(u(:,i),u(:,i)')).^2.*exp(-sqrt(3).*dist2);
                % First three rows evalute the y'*inv*(d/dt)*inv*y part to the gradient
                bdl = (2.*(invC*Kful)-(invC*iKuuKuf')*Kuul)*(iKuuKuf*invC');
                Kuul = iKuuKuf'*Kuul;  % f x u
                Kuul = Kuul(:);        
                Kful = Kful(:);        
                % here is evaluated bd = bdl - tr(B*Kful)+tr(B*Kuul), where B and K* are matrices
                Bdl(i-1) = bdl - 2*sum(B.*Kful)+sum(B.*Kuul);
                % evalute the tr(inv*d/dt) part to the gradient
                Cdl(i-1) = 2.*sum(A.*Kful)-sum(A.*Kuul(:))-2.*sum(W.*Kful)+sum(W.*Kuul);
            end
    ...
    %here evaluate the help arguments for full model
        ...
        % with n=215 
        Cdm = gpcf_st_matern32_trcov(gpcf, x);
        invCv=invC(:);
        b = varargin{1};
        ma2 = gpcf.magnSigma2;
        ...
        s = 1./gpcf.lengthScale.^2;
        dist = 0;
        dist = dist + s(1).*((gminus(x(:,1),x(:,1)')).^2+(gminus(x(:,2),x(:,2)')).^2);
        for i=3:m
            dist = dist + s(i-1).*(gminus(x(:,i),x(:,i)')).^2;
        end
        dist=sqrt(dist);
        D = 3.*ma2.*s(1).*((gminus(x(:,i),x(:,i)')).^2+(gminus(x(:,2),x(:,2)')).^2).*exp(-sqrt(3).*dist);
        Bdl(1) = b'*(D*b);
        Cdl(1) = sum(invCv.*D(:)); % help arguments for lengthScale 
        for i=3:m
            D = 3.*ma2.*s(i-1).*(gminus(x(:,i),x(:,i)')).^2.*exp(-sqrt(3).*dist);
            Bdl(i-1) = b'*(D*b);
            Cdl(i-1) = sum(invCv.*D(:)); % help arguments for lengthScale 
        end
        Bdm = b'*(Cdm*b);
    ...
    function C = gpcf_st_matern32_cov(gpcf, x1, x2)
...
% $$$     else
        ...
        if ~isempty(gpcf.lengthScale)  
            ...
            s2 = s.^2;
            dist=zeros(n1,n2);
            dist = dist + s2(:,1).*((gminus(x1(:,1),x2(:,1)')).^2+(gminus(x1(:,2),x2(:,2)')).^2);
            for j=3:m1
                dist = dist + s2(:,j-1).*(gminus(x1(:,j),x2(:,j)')).^2;
            end
            dist = sqrt(dist);
...
% $$$     end
    ...
    function C = gpcf_st_matern32_trcov(gpcf, x)
...
% $$$     else
        ...
        for i1=2:n
            ...
            for i2=1:i1-1
                ii=i1+(i2-1)*n;
                C(ii)=C(ii)+s2(1).*((x(i1,1)-x(i2,1)).^2+(x(i1,2)-x(i2,2)).^2);       % the covariance function
                for i3=3:m
                    C(ii)=C(ii)+s2(i3-1).*(x(i1,i3)-x(i2,i3)).^2;       % the covariance function
                end
                C(i1n+i2)=C(ii); 
            end
        end
...
% $$$     end
    ...
    function C = gpcf_st_matern32_trvar(gpcf, x)
    ...
    %          Description
    %          RECCF = GPCF_ST_MATERN32_RECAPPEND(RECCF, RI, GPCF) takes old covariance 
    ...
    if nargin == 2
        ...
        reccf.fh_pak = @gpcf_st_matern32_pak;
        reccf.fh_unpak = @gpcf_st_matern32_unpak;
        reccf.fh_e = @gpcf_st_matern32_e;
        reccf.fh_g = @gpcf_st_matern32_g;
        reccf.fh_cov = @gpcf_st_matern32_cov;
        reccf.fh_trcov  = @gpcf_st_matern32_trcov;
        reccf.fh_trvar  = @gpcf_st_matern32_trvar;
        %  gpcf.fh_sampling = @hmc2;
        %  reccf.sampling_opt = hmc2_opt;
        reccf.fh_recappend = @gpcf_st_matern32_recappend;  
        return
    end
...
% $$$ function [Cdm, Cdl] = gpcf_st_matern32_gradhelp(gpcf, x, u)
...
% $$$   % Here we take advantage of the 
% $$$   % symmetry of covariance matrix
% $$$   Cdl=zeros(n1,n1,m1);    % help matrix for gpcf_st_matern32_g
% $$$   Cdm=zeros(n1,n1);    % help matrix for gpcf_st_matern32_g
% $$$   for i1=2:n1
% $$$     i1n=(i1-1)*n1;
% $$$     for i2=1:i1-1
% $$$       ii=i1+(i2-1)*n1;
...
% $$$ end  
% $$$          % Cdm is used in the partial derivatives of gpcf_st_matern32
% $$$          % with respect to log(magnSigma2) (needed in HMC)
% $$$ 
% $$$ % evaluate the help matrix Cdl. This is used in the partial 
% $$$ % derivatives of gpcf_st_matern32 with respect to log(lenghtScale) (needed in HMC)
% $$$ if nargout >1 
% $$$   [n2,m2]=size(u);
% $$$   for i3=1:m1
