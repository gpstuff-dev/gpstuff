function [Ef, Varf, lpy, Ey, Vary] = gpmc2_preds(gp, x, y, varargin)
...
switch gp.type
  case {'LGP', 'LGPC', 'Coxph'}
    error('gpmc2_preds not implemented for this type of likelihood');
  case {'Softmax', 'Multinom'}
    [n,nout] = size(y);
  otherwise
    n = size(y,1);
    nout=size(gp.latentValues(end,:),2)/size(y,1);
end
...
for i1=1:nmc
  ...
  if nargout < 3
    [ef, vf] = gp2_pred(Gp, x, reshape(y(:,i1),n,nout), xt, 'predcf', predcf, 'tstind', tstind);
    Ef(:,i1) = ef(:);
    Varf(:,i1) = vf(:);
  else
    if isfield(gp, 'latentValues')
      ...
      Varf(:,i1) = vf(:);
      if nargout > 3
        [lppy,ey,vy] = Gp.lik.fh.predy(Gp.lik, ef, vf, yt, zt);
        Ey(:,i1) = ey(:);
        Vary(:,i1) = vy(:);
        lpy(:,i1) = lppy(:);
      else
        lppy = Gp.lik.fh.predy(Gp.lik, ef, vf, yt, zt);
        lpy(:,i1) = lppy(:);
      end
    else
      if nargout < 4
        [Ef(:,i1), Varf(:,i1), lpy(:,i1)] = gp2_pred(Gp, x, reshape(y(:,i1),n,nout), xt, 'predcf', predcf, 'tstind', tstind, 'yt', yt);
      else
        [Ef(:,i1), Varf(:,i1), lpy(:,i1), Ey(:,i1), Vary(:,i1)] = gp2_pred(Gp, x, reshape(y(:,i1),n,nout), xt, 'predcf', predcf, 'tstind', tstind, 'yt', yt);
      end
    end
  end
end
