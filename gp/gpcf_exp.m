...
function [g, gdata, gprior]  = gpcf_exp_g(gpcf, x, t, g, gdata, gprior, invC, varargin)
%GPCF_EXP_G Evaluate gradient of error for SE covariance function.
%
%	Descriptioni
...
if isfield(gpcf, 'sparse')
  ...
  switch gpcf.sparse
   case 'FIC'
    ...
    if length(gpcf.lengthScale) == 1
      ...
      Kful = s.*Kfum.*sqrt(dist);
      Kuul = s.*Kuum.*sqrt(dist2);
      ...
      for i=1:nin  
          %                  s = 0.5./gpcf.lengthScale(i);        % set the length
          Kful = s(i).*Kfum.* gminus(x(:,i),u(:,i)').^2;
          Kuul = s(i).*Kuum.* gminus(u(:,i),u(:,i)').^2;
          Kful(dist~=0) = Kful(dist~=0)./dist(dist~=0);
          Kuul(dist2~=0) = Kuul(dist2~=0)./dist2(dist2~=0);
          % First three rows evalute the y'*inv*(d/dt)*inv*y part to the gradient
          bdl = (2.*(invC*Kful)-(invC*iKuuKuf')*Kuul)*(iKuuKuf*invC');
          Kuul = iKuuKuf'*Kuul;  % f x u
          Kuul = Kuul(:);        
  ...
  % Evaluate help arguments for gradien evaluation
...
% $$$ function [Cdm, Cdl] = gpcf_exp_gradhelp(gpcf, x, u)
% $$$ % GP_EXP_GRADHELP     Evaluate help matrices for gradient evaluation.
% $$$ %
% $$$ %         Description
% $$$ %         C = GP_EXP_GRADHELP(GP, TX) takes in covariance function of a Gaussian
% $$$ %         process GP and matrix TX that contains training input vectors to 
% $$$ %         GP. Returns covariance matrix C. Every element ij of C contains  
% $$$ %         covariance between inputs i and j in TX 
% $$$ %
...
% $$$   % Here we take advantage of the 
% $$$   % symmetry of covariance matrix
% $$$   Cdl=zeros(n1,n1,m1);    % help matrix for gpcf_exp_g
% $$$   Cdm=zeros(n1,n1);    % help matrix for gpcf_exp_g
% $$$   for i1=2:n1
% $$$     i1n=(i1-1)*n1;
% $$$     for i2=1:i1-1
% $$$       ii=i1+(i2-1)*n1;
...
% $$$ end  
% $$$          % Cdm is used in the partial derivatives of gpcf_exp
% $$$          % with respect to log(magnSigma2) (needed in HMC)
% $$$ 
% $$$ % evaluate the help matrix Cdl. This is used in the partial 
% $$$ % derivatives of gpcf_exp with respect to log(lenghtScale) (needed in HMC)
% $$$ if nargout >1 
% $$$   [n2,m2]=size(u);
% $$$   for i3=1:m1
