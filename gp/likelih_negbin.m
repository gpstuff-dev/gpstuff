function likelih = likelih_negbin(varargin)
%LIKELIH_NEGBIN    Create a Negative-binomial likelihood structure 
%
%     Description
%       LIKELIH = LIKELIH_NEGBIN('FIELD1', VALUE1, 'FIELD2',
%       VALUE2, ...) Create and initialize Negative-binomial
%       likelihood with fields FIELD1... optionally set to the
%       values VALUE1... The fields that can be modified are:
%
%         disper           = The dispersion parameter
%         disper_prior     = Prior structure for the dispersion
%                            parameter
%  
...
%
%       LIKELIH = LIKELIH_NEGBIN(LIKELIH, 'FIELD1', VALUE1,
...
%                            of likelihood.
%                            Default prior for disper is logunif.
...
%                            moments for EP
%         fh_siteDeriv     = function handle to help gradient evaluations
...
% allow use with or without init and set options
  ...
  switch do
    case 'init'
      ...
      likelih.type = 'Negative-binomial';
  ...
  %
  %   Description
  %   [LIKELIH, W] = LIKELIH_NEGBIN_UNPAK(W, LIKELIH) takes a likelihood 
  %   data structure LIKELIH and extracts the parameters from the vector W
