...
% 
% % obtain MC samples
% [rgp_probit]=gp_mc(gp_probit2, x, y, 'latent_opt', latent_opt, 'nsamples', 5000, 'repeat', 2, 'display', 100);
...
for i=1:length(ind)
  ...
  start=tic;
  [p_probit(:,i), pc_probit(:,i)] = gpep_fact(gp_probit,x,y,fvec_probit(:,i), 'ind', ind(i)); tt_epfact=toc(start);
  ...
  start=tic;
  [p_probit_pred(:,i), pc_probit_pred(:,i)] = gpep_fact(gp_probit,x,y,fvec_probit_pred(:,i),xt, 'ind', ind(i));tt_epfact2=toc(start);
...
% gp_probit_laplace2 = gp_set(gp_probit_laplace, 'latent_method', 'MCMC');
% 
% % set MC options
% latent_opt.repeat = 10;
% 
% % obtain MC samples
% [rgp_probit_laplace]=gp_mc(gp_probit_laplace2, x, y, 'latent_opt', latent_opt, 'nsamples', 5000, 'repeat', 2, 'display', 100);
% rgp_probit_laplace=thin(rgp_probit_laplace,100);
% 
...
for i=1:length(ind)
  ...
  start=tic;[p_probit_laplace(:,i), pc_probit_laplace(:,i), c_probit_laplace(:,i)] = gpla_cm2(gp_probit_laplace,x,y,fvec_probit_laplace(:,i), 'ind', ind(i)); tt_lacm2=toc(start); 
  start=tic;[p_probit_laplace2(:,i), pc_probit_laplace2(:,i), c_probit_laplace2(:,i)] = gpla_fact(gp_probit_laplace,x,y,fvec_probit_laplace(:,i), 'ind', ind(i)); tt_lafact=toc(start);
  ...
  start=tic;[p_pred(:,i), pc_pred(:,i), c_pred(:,i)] = gpla_cm2(gp_probit_laplace,x,y,fvec_probit_laplace_pred(:,i),xt, 'ind', ind(i)); tt_lacm22=toc(start);
  start=tic;[p_pred2(:,i), pc_pred2(:,i), c_pred2(:,i)] = gpla_fact(gp_probit_laplace,x,y,fvec_probit_laplace_pred(:,i),xt, 'ind', ind(i)); tt_lafact2=toc(start);
