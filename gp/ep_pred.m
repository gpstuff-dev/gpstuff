function [y, VarY] = ep_pred(gp, tx, ty, x, varargin)
...
switch gp.type
  case 'FULL'
    ...
    kstarstar=gp_cov_se(0,sigmaf,l);
    ...
    for i1=1:size(K,1)
        ...
        rstar=sqrt(sum((x-repmat(xstar(i1,:),n,1)).^2,2));
        
        % Compute covariance between observations
        kstar=gp_cov_se(rstar,sigmaf,l);
        
        fstar=kstar'*(nutilde-z);
        v=L'\(Stildesqroot*kstar);
        Vfstar(i1,1)=kstarstar-v'*v;
        pistar(i1,1)=normcdf(fstar/sqrt(1+Vfstar(i1)));
    end
