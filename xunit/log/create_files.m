...
% load realValuesSurvival_coxph.mat
% var=who();
% setrandstream(0);
% diary('demo_survival_coxph.txt');
% disp('Running: demo_survival_coxph')
% demo_survival_coxph
% fprintf('\n gp hyperparameters (gp_pak(gp)): \n \n') 
% disp(gp_pak(gp))
% diary off;
% save('realValuesSurvival_coxph.mat', char(var(1)));
% for i=2:length(var)
%   save('realValuesSurvival_coxph.mat', char(var(i)), '-append');
% end
% drawnow;clear;close all;
% 
load realValuesZinegbin.mat
var=who();
setrandstream(0);
diary('demo_zinegbin.txt');
disp('Running: demo_zinegbin')
demo_zinegbin
fprintf('\n gp hyperparameters (gp_pak(gp)): \n \n') 
disp(gp_pak(gp))
diary off;
save('realValuesZinegbin.mat', char(var(1)));
for i=2:length(var)
  save('realValuesZinegbin.mat', char(var(i)), '-append');
end
drawnow;clear;close all;
...
% disp(gp_pak(gp))
% diary off;
% save('realValuesMultinom.mat', char(var(1)));
% for i=2:length(var)
%   save('realValuesMultinom.mat', char(var(i)), '-append');
% end
% drawnow;clear;close all;
% 
